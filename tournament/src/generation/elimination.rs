...
fn order_normalize(list: &Vec<Vec<u64>>) -> Vec<u64> {
    ...
    list.iter().enumerate().for_each(|(index, item)| largest_games.push((*item.iter().max().unwrap(), index as u64)));
...
pub fn new_elim(players: Vec<String>, tuple: u64) -> StableGraph<GraphSet, SetEdge> {
    ...
    if players_count < 2 {
        panic!("Tournament needs at least 2 players, found {}", players_count);
    }
    // TODO: Check for overflowing value
    let full_first_bracket_first_round_sets_count = (players_count as u64).next_power_of_two() / 2;
    let first_bracket_exponent = ((full_first_bracket_first_round_sets_count * 2) as f64).log2() as u64;
    let full_first_bracket_sets_count = (0..first_bracket_exponent).map(|x| 2_u64.pow(x as u32)).sum::<u64>() as usize;

    println!(
        "{:?}: players_count\n{:?}: full_first_bracket_first_round_sets_count\n{:?}: first_bracket_exponent\n{:?}: full_first_bracket_sets_count",
        players_count, full_first_bracket_first_round_sets_count, first_bracket_exponent, full_first_bracket_sets_count,
    );

    let mut sets_graph = StableGraph::<GraphSet, SetEdge>::with_capacity(full_first_bracket_sets_count, full_first_bracket_sets_count - 1);
    // * COMPLETED STEP 1. (z)
    ...
    for i in 1..=full_first_bracket_first_round_sets_count as u64 {
        ...
        sets_graph.add_node(GraphSet {
            bracket: 1,
            game: if triangle(first_bracket_exponent + 1, 2 * i) <= players_count as u64 { game_number } else { 0 },
            round: 1,
            position: i,
            placeholders: (
    ...
    for k in 1..=tuple {
        ...
        if k != 1 {
            if_nec_indices = sets_graph
                ...
                .rev()
                .collect::<Vec<NodeIndex>>();
            if_nec_games = if_nec_indices.iter().fold("".to_owned(), |acc, &index| acc + "W" + &sets_graph[index].game.to_string() + "/");
    ...
    sets_graph.clone().node_indices().for_each(|i| {
        ...
        if sets_graph[i].game == 0 {
            if sets_graph.edges_directed(i, Incoming).find(|e| e.weight().position == 1 && sets_graph[e.source()].game == 0).is_some()
                && sets_graph.edges_directed(i, Incoming).find(|e| e.weight().position == 2 && sets_graph[e.source()].game == 0).is_some()
            {
                ...
                let winner_placeholder = if sets_graph.edges_directed(i, Incoming).find(|e| e.weight().position == 1 && sets_graph[e.source()].game == 0).is_none() {
                    ...
                    if e.weight().outcome == "loser" {
                        ...
                        if e.weight().position == 1 {
                            if sets_graph[i].bracket > 1 && winner_placeholder.starts_with("L") {
                                sets_graph.add_edge(
                                    sets_graph.edges_directed(i, Incoming).find(|f| f.weight().outcome == "loser").unwrap().source(),
                                    e.target(),
                                    SetEdge {
                                        outcome: "loser".to_owned(),
                                        position: 1,
    ...
    sets_graph.clone().node_indices().for_each(|i| {
        if sets_graph[i].round != 1 && sets_graph[i].round < sets_graph[sets_graph.node_indices().rfind(|&n| sets_graph[n].bracket == sets_graph[i].bracket).unwrap()].round - sets_graph[i].bracket
        {
            if sets_graph[i].position == 1 {
                sets_graph
                    .node_indices()
                    .filter(|&n| sets_graph[n].bracket == sets_graph[i].bracket && sets_graph[n].round == sets_graph[i].round)
                    .for_each(|n| game_numbers.push(sets_graph.edges_directed(n, Incoming).map(|e| sets_graph[e.source()].game).collect::<Vec<u64>>()));
                game_number_offsets = order_normalize(&game_numbers);
                game_numbers = Vec::new();
            }
            println!(
                "connected_bracket: {}, connected_round: {}, bracket: {}, round: {}, position: {}, game: {}, game_number_offset: {}, game_number_offsets: {:?}",
                sets_graph[i].bracket
                    - if sets_graph[i].bracket == 1 || sets_graph[i].bracket == 2 && (sets_graph[i].round - 1) % 2 == 0 || (sets_graph[i].round - 1) % sets_graph[i].bracket == 2 {
                        0
                    } else {
                        1
                    },
                if sets_graph[i].bracket == 1 || sets_graph[i].bracket == 2 && (sets_graph[i].round - 1) % 2 == 0 || (sets_graph[i].round - 1) % sets_graph[i].bracket == 2 {
                    ...
                    sets_graph[sets_graph.edges_directed(i, Incoming).find(|f| f.weight().outcome == "loser").unwrap().source()].round
                },
                sets_graph[i].bracket,
                sets_graph[i].round,
            ...
            if sets_graph[i].game != 0 {
                sets_graph[i].game = sets_graph[i].game + game_number_offsets[sets_graph[i].position as usize - 1] - sets_graph[i].position;
            } else {
                sets_graph
                    .node_indices()
                    .filter(|&g| {
                        sets_graph[g].game != 0
                            && if sets_graph[g].bracket == sets_graph[i].bracket && sets_graph[g].round == sets_graph[i].round {
                                game_number_offsets[sets_graph[g].position as usize - 1] > game_number_offsets[sets_graph[i].position as usize - 1]
                            } else {
                                g > i
                            }
                    })
    ...
    let mut losers_parent_round: HashMap<u64, u64> = HashMap::new();
    for j in 1..=tuple {
        for i in 1..=sets_graph[sets_graph.node_indices().rfind(|&n| sets_graph[n].bracket == j).unwrap()].round {
            if sets_graph
                .node_indices()
                .filter(|&n| sets_graph[n].bracket == j && sets_graph[n].round == i)
                .all(|f| sets_graph[f].game != 0)
            {
                if sets_graph
                    .node_indices()
                    .filter(|&n| sets_graph[n].bracket == j && sets_graph[n].round == i)
                    ...
                    > sets_graph
                        .node_indices()
                        .filter(|&n| sets_graph[n].bracket == j && sets_graph[n].round == i + 1)
                        .collect::<Vec<NodeIndex>>()
                        .len()
                {
                    losers_parent_round.insert(j, i);
                } else {
                    losers_parent_round.insert(j, i + 1);
                }
            }
        }
    }
    ...
    for j in 1..=tuple {
        ...
        let init_i: u64 = sets_graph[sets_graph.node_indices().rfind(|&n| sets_graph[n].bracket == j).unwrap()].round;
        for i in 1..=sets_graph[sets_graph.node_indices().rfind(|&n| sets_graph[n].bracket == j).unwrap()].round {
            if sets_graph.node_indices().find(|&n| sets_graph[n].bracket == j && sets_graph[n].round == i - i_offset).is_none() {
    ...
    // * COMPLETED STEP 1. (h)
    sets_graph.clone().node_indices().for_each(|i| {
        if sets_graph.edges_directed(i, Incoming).find(|e| e.weight().position == 1).is_some() {
            sets_graph[i].placeholders.0 = sets_graph.edges_directed(i, Incoming).filter(|e| e.weight().position == 1).fold("".to_owned(), |mut acc, e| {
                acc.insert_str(
                    0,
                    &(if e.weight().outcome == "winner" { "W" } else { "L" }.to_owned() + &sets_graph[e.source()].game.to_string() + if acc != "" { "/" } else { "" }),
                );
                acc
            });
        }
        if sets_graph.edges_directed(i, Incoming).find(|e| e.weight().position == 2).is_some() {
            sets_graph[i].placeholders.1 = sets_graph.edges_directed(i, Incoming).filter(|e| e.weight().position == 2).fold("".to_owned(), |mut acc, e| {
                acc.insert_str(
                    0,
                    &(if e.weight().outcome == "winner" { "W" } else { "L" }.to_owned() + &sets_graph[e.source()].game.to_string() + if acc != "" { "/" } else { "" }),
                );
    ...
    // ! COMPLETE STEP 1. (k)
    for j in 1..=tuple {
        // let offset = (sets_graph
        //     .node_indices()
        //     .filter(|&n| {
        //         sets_graph[n].bracket == j
        //             && ((sets_graph.edges_directed(n, Incoming).all(|f| f.weight().outcome == "winner")
        //                 && sets_graph.edges_directed(n, Incoming).any(|f| sets_graph[f.source()].bracket != j)
        //                 && sets_graph.edges_directed(n, Incoming).find(|f| sets_graph[f.source()].bracket == j).unwrap().weight().position == 2)
        //                 || (sets_graph
        ...
        //                     .len()
        //                     == 1
        //                     && sets_graph.edges_directed(n, Incoming).find(|f| f.weight().outcome == "winner").unwrap().weight().position == 2)
        //                 || (sets_graph
        //                     .edges_directed(n, Incoming)
        //                     .filter(|f| f.weight().outcome == "loser")
        //                     .collect::<Vec<EdgeReference<SetEdge>>>()
        //                     .len()
        //                     == 2
        ...
        //                 && ((sets_graph.edges_directed(n, Incoming).all(|f| f.weight().outcome == "winner")
        //                     && sets_graph.edges_directed(n, Incoming).any(|f| sets_graph[f.source()].bracket != j)
        //                     && sets_graph.edges_directed(n, Incoming).find(|f| sets_graph[f.source()].bracket == j).unwrap().weight().position == 1)
        //                     || (sets_graph
        //                         .edges_directed(n, Incoming)
        //                         .filter(|f| f.weight().outcome == "winner")
        //                         .collect::<Vec<EdgeReference<SetEdge>>>()
        //                         .len()
        //                         == 1
        //                         && sets_graph.edges_directed(n, Incoming).find(|f| f.weight().outcome == "winner").unwrap().weight().position == 1))
        //         })
        //         .collect::<Vec<NodeIndex>>()
        //         .len()) as u64;
        // println!("j: {}", j);
